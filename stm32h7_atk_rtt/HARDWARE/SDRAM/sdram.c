...
{ 
    ...
    sdtimereg|=7<<4;                //退出自刷新延迟为8个时钟周期,tXSR=72ns(min)
    sdtimereg|=4<<8;                //自刷新时间为5个时钟周期,tRAS=42ns(min)
    sdtimereg|=6<<12;                //行循环延迟为7个时钟周期,tRC=60ns(min)
    ...
    FMC_Bank5_6->SDRTR=839<<1;        //设置刷新频率计数器
} 
