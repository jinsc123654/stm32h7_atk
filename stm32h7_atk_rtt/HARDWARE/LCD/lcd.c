...
{      
    ...
    }else
    {
        ...
        FMC_Bank1->BTCR[1]|=15<<0;        //地址建立时间(ADDSET)为15个fmc_ker_ck 1/220M=4.5ns*15=67.5ns    
        //因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片。
        FMC_Bank1->BTCR[1]|=78<<8;      //数据保存时间(DATAST)为78个fmc_ker_ck=4.5*78=351ns
        //写时序控制寄存器  
        FMC_Bank1E->BWTR[0]|=0<<28;     //模式A                                      
        FMC_Bank1E->BWTR[0]|=15<<0;        //地址建立时间(ADDSET)为15个fmc_ker_ck=67.5ns    
        //15个fmc_ker_ck（fmc_ker_ck=220Mhz）,某些液晶驱动IC的写信号脉宽，最少也得50ns。       
        FMC_Bank1E->BWTR[0]|=15<<8;        //数据保存时间(DATAST)为15个fmc_ker_ck=67.5ns    
        //使能BANK1,区域1
    ...
    if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
    {
        ...
        FMC_Bank1E->BWTR[0]|=4<<0;         //地址建立时间(ADDSET)为4个fmc_ker_ck=18ns       
        FMC_Bank1E->BWTR[0]|=4<<8;         //数据保存时间(DATAST)为4个fmc_ker_ck=18ns
    } 
